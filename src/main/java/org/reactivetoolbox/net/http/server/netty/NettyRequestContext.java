...
class NettyRequestContext implements RequestContext {
    ...
    NettyRequestContext(final ChannelHandlerContext context, final FullHttpRequest request) {
    ...
    public void writeResponse(final HttpResponseStatus status,
        ...
        context.writeAndFlush(response).addListener(keepAlive ? (ch) -> {} : ChannelFutureListener.CLOSE);
    }
    ...
    private FullHttpResponse buildResponse(final HttpResponseStatus status,
        ...
        setKeepAlive(headers).set(SERVER, SERVER_NAME)
    ...
    private HttpHeaders setKeepAlive(final HttpHeaders headers) {
        if (keepAlive) {
